...
### 1.1 Project Setup
- [ ] 1.1.1 Initialize Go module (go 1.22+)
- [ ] 1.1.2 Set up package structure (cmd/, pkg/, test/)
- [ ] 1.1.3 Add go.mod with core dependencies (pion, gorilla/websocket)
- [ ] 1.1.4 Create Makefile for build/test/lint
- [ ] 1.1.5 Set up GitHub Actions CI/CD pipeline

### 1.2 HPB Signaling Client
- [ ] 1.2.1 Define HPB protocol message types (hello, incall, room, message, bye)
- [ ] 1.2.2 Implement WebSocket connection with TLS support
- [ ] 1.2.3 Implement HMAC-SHA256 authentication (LT_INTERNAL_SECRET)
- [ ] 1.2.4 Implement reconnection logic with exponential backoff
- [ ] 1.2.5 Implement session resumption (resumeid)
- [ ] 1.2.6 Implement participant tracking
- [ ] 1.2.7 Write unit tests with mock HPB server
- [ ] 1.2.8 Document HPB protocol flow

### 1.3 WebRTC Manager
- [ ] 1.3.1 Define RTC peer configuration and ICE candidate handling
- [ ] 1.3.2 Implement PeerConnection creation with STUN/TURN servers from HPB
- [ ] 1.3.3 Implement SDP offer/answer negotiation (JSEP)
- [ ] 1.3.4 Implement audio track extraction from peer connection
- [ ] 1.3.5 Implement OnICECandidate callback
- [ ] 1.3.6 Implement connection state monitoring (connected, disconnected, failed)
- [ ] 1.3.7 Write unit tests for peer connection lifecycle
- [ ] 1.3.8 Test audio frame delivery format (channels, sample rate)

### 1.4 Audio Pipeline
- [ ] 1.4.1 Define audio buffer structure (capacity, format)
- [ ] 1.4.2 Implement audio frame buffering from WebRTC tracks
- [ ] 1.4.3 Implement resampling (48kHz → 24kHz) using go-audio
- [ ] 1.4.4 Implement format conversion (int16 PCM → float32 PCM)
- [ ] 1.4.5 Implement 200ms chunk buffering logic
- [ ] 1.4.6 Implement bounded channel for audio output (natural backpressure)
- [ ] 1.4.7 Write tests for resampling accuracy
- [ ] 1.4.8 Write tests for format conversion edge cases
...
- [ ] 2.1.1 Define Modal authentication (workspace key/secret)
- [ ] 2.1.2 Implement WebSocket connection to Modal endpoint
- [ ] 2.1.3 Implement binary frame sending (float32 PCM)
- [ ] 2.1.4 Implement JSON response parsing (text, final, vad_end fields)
...
- [ ] 2.2.1 Define transcript message structure
- [ ] 2.2.2 Implement result parsing from Modal responses
...
### 3.1 Session Manager
- [ ] 3.1.1 Define Room state structure (token, participants, transcribers)
- [ ] 3.1.2 Implement room creation/deletion lifecycle
- [ ] 3.1.3 Implement speaker/participant tracking
- [ ] 3.1.4 Implement target (who gets transcripts) management
- [ ] 3.1.5 Implement goroutine spawn/cleanup with WaitGroup
- [ ] 3.1.6 Implement graceful shutdown with context cancellation
...
- [ ] 3.2.1 Implement HPB → WebRTC signaling flow
...
### 3.3 API Bridge
- [ ] 3.3.1 Define HTTP/gRPC endpoints (matching Python contract)
- [ ] 3.3.2 Implement POST /api/v1/call/transcribe
- [ ] 3.3.3 Implement status/health endpoints
